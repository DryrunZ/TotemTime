...
// ---- game settings (language, difficulty) — options come from data: locale docs and scoring.modes ----
const modesOf = (game) => { const m = Object.keys((game.scoring && game.scoring.modes) || {}); return m.length ? m : ["easy"]; };
const langsOf = async (gameId) => { const s = await db.collection(`games/${gameId}/locales`).get(); return s.docs.map(d => d.id); };
const pickMode = (game, want) => { const ms = modesOf(game); return ms.includes(want) ? want : ms[0]; };
const pickLang = async (game, gameId, want) => {
  const ls = await langsOf(gameId); const def = game.defaultLanguage || "he";
  if (want && ls.includes(want)) return want;
  return ls.includes(def) ? def : (ls[0] || def);
};
...
exports.judge = onCall(async (req) => {
  ...
  if (seatId && seatId !== callerUid && !(await isAdmin(callerUid))) throw new HttpsError("permission-denied", "that seat is not yours");
  ...
  // ---- buildCollage: after finish. Reads every photo under instances/{id}/photos/, renders, stores collage.jpg.
  // Rebuilt on every call, so late selfies just improve it. Data-gated: instance rooms only.
  if (action === "buildCollage") {
    const room = (await roomRef.get()).data();
    if (!room || !room.instanceId) throw new HttpsError("failed-precondition", "no instance on this room");
    const seated = room.seats && room.seats[callerUid];
    if (!seated && room.buyerUid !== callerUid && !(await isAdmin(callerUid))) throw new HttpsError("permission-denied", "players only");
    const iRef = db.doc(`instances/${room.instanceId}`);
    const inst = (await iRef.get()).data() || {};
    const bucket = getStorage().bucket();
    const [files] = await bucket.getFiles({ prefix: `instances/${room.instanceId}/photos/` });
    const photoFiles = files.filter(f => !f.name.endsWith("/"));
    if (!photoFiles.length) throw new HttpsError("failed-precondition", "no photos yet");
    const photos = [];
    for (const f of photoFiles) { const [b] = await f.download(); photos.push(b); }
    const lang = inst.language || room.language || game.defaultLanguage || "he";
    const loc = (await db.doc(`games/${gameId}/locales/${lang}`).get()).data() || {};
    const gameTitle = (game.title_key && loc[game.title_key]) || game.title || gameId;
    const cz = inst.customization || {};
    const title = String(cz.collage_title || cz.owner_name || cz.team_name || gameTitle).trim();
    const d = new Date(inst.finishedAt || Date.now());
    const dateStr = d.toLocaleDateString(lang === "he" ? "he-IL" : "en-GB", { day: "numeric", month: "long", year: "numeric" });
    const pts = typeof inst.finalPoints === "number" ? inst.finalPoints : room.points;
    const subtitle = [gameTitle, dateStr, lang === "he" ? `${pts} נקודות` : `${pts} points`].join(" · ");
    const jpg = await renderCollage(photos, { title, subtitle, footer: "TotemTime · totemtime.com" });
    const token = randomUUID();
    const dest = bucket.file(`instances/${room.instanceId}/collage.jpg`);
    await dest.save(jpg, { contentType: "image/jpeg", metadata: { cacheControl: "public,max-age=60", metadata: { firebaseStorageDownloadTokens: token } } });
    const url = `https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/${encodeURIComponent(dest.name)}?alt=media&token=${token}`;
    await iRef.set({ collageUrl: url, collageAt: nowIso(), collagePhotos: photos.length }, { merge: true });
    return { collageUrl: url, photos: photos.length };
  }

  // ---- myFinish: the caller's own coupon + the group collage. Works for anonymous players,
  // who have no users/{uid} doc and cannot read coupons/ directly. Seated callers only.
  if (action === "myFinish") {
    const room = (await roomRef.get()).data();
    if (!room) throw new HttpsError("not-found", "room not found");
    if (!room.seats || !room.seats[callerUid]) throw new HttpsError("permission-denied", "players only");
    let coupon = null, collageUrl = null, finalPoints = null, durationMs = null;
    if (room.instanceId) {
      const inst = (await db.doc(`instances/${room.instanceId}`).get()).data() || {};
      collageUrl = inst.collageUrl || null;
      finalPoints = typeof inst.finalPoints === "number" ? inst.finalPoints : (room.points ?? null);
      durationMs = inst.durationMs ?? null;
      const qs = await db.collection("coupons")
        .where("instanceId", "==", room.instanceId)
        .where("ownerUid", "==", callerUid).limit(1).get();
      if (!qs.empty) {
        const c = qs.docs[0].data();
        coupon = { code: c.code, discountPct: c.discountPct, expiresAt: c.expiresAt, used: !!c.used };
      }
    }
    return { coupon, collageUrl, finalPoints, durationMs, anon: !!(req.auth && req.auth.token && req.auth.token.provider_id === "anonymous") };
  }

  // ---- setMode: difficulty on the room (data the judge already reads via __mode). Admin anytime; director before start. ----
  if (action === "setMode") {
    const room = (await roomRef.get()).data() || {};
    const admin = await isAdmin(callerUid);
    if (!admin && room.buyerUid !== callerUid) throw new HttpsError("permission-denied", "admins or the director only");
    if (!admin && room.phase !== "lobby") throw new HttpsError("failed-precondition", "locked once the game has started");
    const md = pickMode(game, req.data && req.data.mode);
    await roomRef.update({ mode: md });
    return { mode: md };
  }
  ...
  if (action === "join") {
    ...
    return await db.runTransaction(async (tx) => {
      ...
      if (!room) throw new HttpsError("not-found", "room not found");
      const seats = room.seats || {};
      const av = Number.isInteger(req.data && req.data.av) ? req.data.av : 0;
      if (!seats[seatId]) {
        const cap = Number(room.maxSeats) || 0;
        if (cap && Object.keys(seats).length >= cap) throw new HttpsError("resource-exhausted", "room is full");
        const joinIndex = Object.keys(seats).length + 1;
        seats[seatId] = { joinIndex, name, av, ready: false, lastSeen: Date.now() };
        tx.update(roomRef, { seats });
      } else if (seats[seatId].name !== name || seats[seatId].av !== av) {
        seats[seatId].name = name; seats[seatId].av = av;
        tx.update(roomRef, { seats });
      }
...
exports.claimGame = onCall(async (req) => {
  ...
  const { gameId, seatCount, language, claimCode, mode } = req.data || {};
  ...
  const scoring = game.scoring || {};
  const lang = await pickLang(game, gameId, language);
  const md = pickMode(game, mode);
  await db.doc(`rooms/${code}`).set({
    gameId, phase: "lobby", step: 0, points: scoring.start ?? 100, buyerUid: uid,
    startedAt: null, solved: {}, hintsUsed: {}, seats: {}, flags: {}, prog: {},
    mode: md, language: lang,
  });

  const instRef = db.collection("instances").doc();
  await db.doc(`rooms/${code}`).update({ instanceId: instRef.id, maxSeats: seats });
  await instRef.set({
    gameId, buyerUid: uid, seatCount: seats,
    language: lang, mode: md, status: "open",
...
exports.addSeat = onCall(async (req) => {
  ...
  return await db.runTransaction(async (tx) => {
    ...
    if (inst.code) tx.update(db.doc(`rooms/${inst.code}`), { maxSeats: inst.seatCount + 1 });
...
// ---- setInstanceSettings: director edits language/difficulty before the room starts. Writes instance + room together. ----
exports.setInstanceSettings = onCall(async (req) => {
  const uid = req.auth && req.auth.uid;
  if (!uid) throw new HttpsError("unauthenticated", "sign in first");
  const { instanceId, language, mode } = req.data || {};
  if (!instanceId) throw new HttpsError("invalid-argument", "instanceId required");
  const iRef = db.doc(`instances/${instanceId}`);
  const inst = (await iRef.get()).data();
  if (!inst) throw new HttpsError("not-found", "instance not found");
  if (inst.buyerUid !== uid && !(await isAdmin(uid))) throw new HttpsError("permission-denied", "director only");
  if (inst.status === "played" || inst.status === "finished") throw new HttpsError("failed-precondition", "locked once the game has started");
  const game = (await db.doc(`games/${inst.gameId}`).get()).data() || {};
  const lang = await pickLang(game, inst.gameId, language || inst.language);
  const md = pickMode(game, mode || inst.mode);
  await iRef.update({ language: lang, mode: md });
  if (inst.code) await db.doc(`rooms/${inst.code}`).set({ language: lang, mode: md }, { merge: true });
  return { language: lang, mode: md };
});
